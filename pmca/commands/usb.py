...
def _printTweaks(tweaks):
 for id, desc, status, value in tweaks:
  print('%-16s[%s] %s' % (id, ('X' if status else ' '), desc))
  print('                %s' % value)


def _runTweakCommand(backend, enableAll=False, enable=None, disable=None, listOnly=False, tolerateApplyWarning=False):
 enable = enable or []
 disable = disable or []
 conflictingTweaks = sorted(set(enable) & set(disable))
 if conflictingTweaks:
  raise Exception('Cannot enable and disable the same tweak: %s' % ', '.join(conflictingTweaks))

 backend.start()
 try:
  tweakInterface = TweakInterface(backend)
  ...
  if not enableAll and not enable and not disable:
   print('')
   print('No changes requested. Use --all, --enable, or --disable.')
   return

  available = {id: desc for id, desc, status, value in tweaks}
  for id in enable + disable:
   if id not in available:
    raise Exception('Unknown or unavailable tweak: %s' % id)

  print('')
  if enableAll:
   for id in available:
    print('Enabling %s' % id)
    tweakInterface.setEnabled(id, True)
  ...
  for id in disable:
   print('Disabling %s' % id)
   tweakInterface.setEnabled(id, False)

  print('Applying tweaks...')
  try:
   tweakInterface.apply()
  except Exception as e:
   if tolerateApplyWarning and str(e).strip() == 'Cannot overwrite backup':
    print('Warning: Cannot overwrite backup')
    print('The requested tweak may already have been written. Power-cycle the camera, reconnect, and run the command again to verify.')
   else:
    raise
  else:
   print('Done')
...
def tweakCommand(mode='updater', enableAll=False, enable=None, disable=None, listOnly=False, model=None, fdatFile=None, driverName=None, tolerateApplyWarning=False):
 """List or apply tweaks without opening the interactive platform shell."""
 if mode == 'updater':
  updaterShellCommand(model, fdatFile, driverName, lambda dev: _runTweakCommand(
   UsbPlatformBackend(dev),
   enableAll,
   enable,
   disable,
   listOnly,
   tolerateApplyWarning,
  ))
 elif mode == 'service':
  if model or fdatFile:
   raise Exception('--model and --fdat can only be used with updater mode')
  senserShellCommand(driverName, lambda dev: _runTweakCommand(
   SenserPlatformBackend(dev),
   enableAll,
   enable,
   disable,
   listOnly,
  ...
  raise Exception('Unknown tweak mode: %s' % mode)


def unlockLanguagesCommand(driverName='libusb', assumeYes=False):
 """Enable all menu languages through service mode."""
 if not assumeYes:
  print('This will write the camera language backup setting.')
  print('Make sure the camera battery is charged and USB is set to Mass Storage.')
